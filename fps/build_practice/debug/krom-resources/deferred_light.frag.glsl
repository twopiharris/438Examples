...
float PCFCube(samplerCubeShadow shadowMapCube, vec3 lp, inout vec3 ml, float bias, vec2 lightProj_1, vec3 n)
{
    ...
    float compare = _271 - (bias * 1.5);
    ml += ((n * bias) * 20.0);
    vec4 _288 = vec4(ml, compare);
    float result = texture(shadowMapCube, vec4(_288.xyz, _288.w));
    vec4 _300 = vec4(ml + vec3(0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_300.xyz, _300.w));
    vec4 _314 = vec4(ml + vec3(-0.001000000047497451305389404296875, 0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_314.xyz, _314.w));
    vec4 _327 = vec4(ml + vec3(0.001000000047497451305389404296875, -0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_327.xyz, _327.w));
    vec4 _340 = vec4(ml + vec3(0.001000000047497451305389404296875, 0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_340.xyz, _340.w));
    vec4 _353 = vec4(ml + vec3(-0.001000000047497451305389404296875, -0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_353.xyz, _353.w));
    vec4 _366 = vec4(ml + vec3(0.001000000047497451305389404296875, -0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_366.xyz, _366.w));
    vec4 _379 = vec4(ml + vec3(-0.001000000047497451305389404296875, 0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_379.xyz, _379.w));
    vec4 _392 = vec4(ml + vec3(-0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_392.xyz, _392.w));
    return result / 9.0;
}
...
vec3 sampleLight(vec3 p, vec3 n, vec3 v, float dotNV, vec3 lp, vec3 lightCol, vec3 albedo, float rough, float spec, vec3 f0, int index, float bias)
{
    ...
    direct *= lightCol;
    vec3 param = -l;
    float _448 = PCFCube(shadowMapPoint[0], ld, param, bias, lightProj, n);
    direct *= _448;
    return direct;
}
...
void main()
{
    ...
    {
        _635 = octahedronWrap(g0.xy);
    }
    n = vec3(_635.x, _635.y, n.z);
    n = normalize(n);
    ...
    vec3 _746 = fragColor.xyz * textureLod(ssaotex, texCoord, 0.0).x;
    fragColor = vec4(_746.x, _746.y, _746.z, fragColor.w);
    int param_2 = 0;
    float param_3 = pointBias;
    vec3 _769 = fragColor.xyz + sampleLight(p, n, v, dotNV, pointPos, pointCol, albedo, roughness, occspec.y, f0, param_2, param_3);
    fragColor = vec4(_769.x, _769.y, _769.z, fragColor.w);
    fragColor.w = 1.0;
}
