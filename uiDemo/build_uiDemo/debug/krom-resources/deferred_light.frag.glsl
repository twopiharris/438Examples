...
float PCFCube(samplerCubeShadow shadowMapCube, vec3 lp, inout vec3 ml, float bias, vec2 lightProj_1, vec3 n)
{
    ...
    float compare = _300 - (bias * 1.5);
    ml += ((n * bias) * 20.0);
    vec4 _317 = vec4(ml, compare);
    float result = texture(shadowMapCube, vec4(_317.xyz, _317.w));
    vec4 _329 = vec4(ml + vec3(0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_329.xyz, _329.w));
    vec4 _343 = vec4(ml + vec3(-0.001000000047497451305389404296875, 0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_343.xyz, _343.w));
    vec4 _356 = vec4(ml + vec3(0.001000000047497451305389404296875, -0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_356.xyz, _356.w));
    vec4 _369 = vec4(ml + vec3(0.001000000047497451305389404296875, 0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_369.xyz, _369.w));
    vec4 _382 = vec4(ml + vec3(-0.001000000047497451305389404296875, -0.001000000047497451305389404296875, 0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_382.xyz, _382.w));
    vec4 _395 = vec4(ml + vec3(0.001000000047497451305389404296875, -0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_395.xyz, _395.w));
    vec4 _408 = vec4(ml + vec3(-0.001000000047497451305389404296875, 0.001000000047497451305389404296875, -0.001000000047497451305389404296875), compare);
    result += texture(shadowMapCube, vec4(_408.xyz, _408.w));
    vec4 _421 = vec4(ml + vec3(-0.001000000047497451305389404296875), compare);
...
vec3 sampleLight(vec3 p, vec3 n, vec3 v, float dotNV, vec3 lp, vec3 lightCol, vec3 albedo, float rough, float spec, vec3 f0, int index, float bias)
{
    ...
    direct *= lightCol;
    vec3 param = -l;
    float _477 = PCFCube(shadowMapPoint[0], ld, param, bias, lightProj, n);
    direct *= _477;
    return direct;
}
...
void main()
{
    ...
    envl = pow(envl, vec3(2.2000000476837158203125));
    prefilteredColor = pow(prefilteredColor, vec3(2.2000000476837158203125));
    envl *= albedo;
    ...
    vec3 _820 = fragColor.xyz * textureLod(ssaotex, texCoord, 0.0).x;
    fragColor = vec4(_820.x, _820.y, _820.z, fragColor.w);
    int param_2 = 0;
    float param_3 = pointBias;
    vec3 _843 = fragColor.xyz + sampleLight(p, n, v, dotNV, pointPos, pointCol, albedo, roughness, occspec.y, f0, param_2, param_3);
